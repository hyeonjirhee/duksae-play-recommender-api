...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          "당신은 기분과 상황에 맞춰 놀이를 제안해주는 AI 놀이 전문가입니다. 추천은 밝고 긍정적이며 유쾌하게 제안해주세요. 추천은 짧고 간결하게, 부정적인 표현은 피하고, 놀이 이름이나 장소·준비물을 구체적으로 언급해주세요. partner에 이름을 언급하면 그건 본인이 아닌 친구 이름이에요.",
      },
    });
